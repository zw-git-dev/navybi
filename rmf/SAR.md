...
| 8 | CM-6 | No formal configuration hardening baseline for the host OS, Python runtime, or Streamlit server (e.g., no TLS on the app itself, default bind settings). | Low (as-built, single-host demo); Moderate (once network-exposed) | Apply a hardening checklist before any network-facing deployment. |
